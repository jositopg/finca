...
- **Agua y luz (`reparto.aguaLuz`)**: en cada propiedad, incluidos / no incluidos / cupo mensual conjunto (p.ej. 30 € de agua+luz juntos). `desgloseSuministrosEnRango` / `resumenEnRango` / `sumGastosEnRango` son la fuente de los totales: la parte no incluida **no cuenta como gasto** (la factura se queda informativa) y sale como `inquilino` para repercutirla. El cupo es por mes civil y conjunto, no por factura. `cuotaSuministro` reparte ese cupo entre las facturas del mes. No se generan ingresos automáticos de "Agua/Electricidad (repercutida)". Si no hay `aguaLuz`, se unifica el legado `reparto.agua`+`reparto.luz` cuando coinciden; si no, cada concepto usa `calcularReparto`. Basuras e IBI siguen por concepto. Exportación a Sheets: el cupo conjunto mensual no se puede expresar por fila (queda en columnas J/K).
...
- **`alquilerACobrar` / `deudaInquilino`**: en locales la deuda y el aviso de cobro usan la **neta** (lo que entra en el banco), no la bruta. Un cobro parcial no cierra el mes (`rentaPendiente` suma importes). `diaCobro` (1-28, default 5) mueve el aviso. El cobro de renta guarda el **mes que paga** en `periodoInicio`/`periodoFin` (la fecha es cuándo entra el dinero). Un atraso de enero cobrado en marzo no tapa marzo. Cobros viejos sin periodo siguen restando del total.
- **`rendimientoIrpfPropiedad`**: única fuente de la pestaña Fiscal y del estimador. Excluye fianzas; locales a base imponible; prorratea con `importeEnRango`; no deduce `Hipoteca / Financiación` (sí `Intereses hipoteca`); amortización prorrateada por meses alquilados del año. Gastos de agua/luz: solo la parte incluida (`sumGastosEnRango` con `{ irpf: true }`). Los ingresos "Agua/Electricidad (repercutida)" no cuentan como rendimiento.
...
- **Huecos mensuales** (`huecosMensuales`): en cada alquiler, avisos de renta/agua/luz que **no cubren ese mes** (por `periodoInicio`/`periodoFin` o, si no hay periodo, por fecha). No importa cuándo se registró el movimiento. Últimos 6 meses desde la ocupación. Jose puede quitar un aviso (`huecosMensualesOmitidos`, JSONB). Columna `huecos_mensuales_omitidos`.
- **Gastos fijos**: `gastoRecurrenteId` en la transacción + unique index `(propiedad_id, gasto_recurrente_id, fecha)` para no duplicar entre dispositivos. `generarGastosPendientes` rellena desde el alta hasta el mes actual, salvo **Tasa de basuras**, que cubre todo el año civil (p.ej. 2026 entero). Si hay tasa fija y no está marcada como incluida en la renta, `repartoEfectivo` la trata como no incluida (a repercutir, no como gasto). Las lecturas de Supabase **paginan** de 1000 en 1000.
...
- **Documentos de alquiler**: solo **locales** generan factura (serie F, IGIC). Vivienda/garaje no llevan factura ni recibo en la app. El número usa el año del movimiento. Hay un `F-2026-001` duplicado en prod — no se ha puesto UNIQUE en DB.
...
- **Dashboard**: operativo del día a día — resumen del mes/año, tareas pendientes, cobros de renta, acceso rápido a facturas de agua/luz, listado de propiedades. **Los avisos (contrato por vencer, revisión de renta, certificado energético, fianza) NO van agregados aquí** — viven dentro de la ficha y tarjeta de cada propiedad. El agua/luz a repercutir no se lista en Inicio: está en la ficha de cada propiedad.
- **Estadísticas**: gráficos agregados (evolución del rendimiento neto por año, gastos por categoría, ranking de rentabilidad entre propiedades, agua y luz a repercutir en el año). Cualquier gráfico/estadística agregada nueva va aquí, no en el Dashboard. La app solo empieza a acumular histórico útil desde 2026, así que la evolución anual (`EvolucionAnual`, requiere ≥2 años con datos) no se mostrará hasta que haya suficiente histórico. El a repercutir del año va aquí (`RepercutirSuministros`: agua/luz no incluidos, basuras, IBI…); el operativo, en la ficha (`RepercutirSuministrosFicha`, plegado). Tasa de basuras como gasto fijo se replica en todos los meses del año civil.
